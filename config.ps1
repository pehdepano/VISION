############################################################################
#Project: Create config.csv file
...
#Create header in a new file "config.csv"
Write-Output ('"Nome_Ponto","Estrutura_Enviadas"') | Out-File (".\config.csv") -Encoding utf8
...
#fill "config.csv" with selected data
	Write-Output ('"{0}.1","{1}"' -f $name2,$enviadas1) | Out-File (".\config.csv") -Append -Encoding utf8
	if ($IP2) {
	Write-Output ('"{0}.2","{1}"' -f $name2,$enviadas2) | Out-File (".\config.csv") -Append -Encoding utf8}
	if ($IP3) {
	Write-Output ('"{0}.3","{1}"' -f $name2,$enviadas3) | Out-File (".\config.csv") -Append -Encoding utf8}
	if ($IP4) {
	Write-Output ('"{0}.4","{1}"' -f $name2,$enviadas4) | Out-File (".\config.csv") -Append -Encoding utf8}
}
